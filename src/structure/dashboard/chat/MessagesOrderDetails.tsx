...
import { createOrder, getOrder, updateOrder } from "../../../model/orderModel";
...
export default function MessagesOrderDetails() {
  ...
  const { data: orderData, isSuccess } = useQuery({
    ...
    queryFn:
      orderId === "new"
        ? () => {
            const order = (partnerId ? { vendorId: Number(partnerId) } : {}) as Order;
            return order;
          }
  ...
  const updateOrderMutation = useMutation({
    mutationFn: orderId === "new" ? createOrder : updateOrder,
    onSuccess: (data) => {
          ...
          <div className="flex flex-wrap gap-6 justify-between w-full">
            <button
              ...
              className={classNames(
                "text-md ml-4 flex max-w-lg gap-4 animate-[wiggle_1s_ease-in-out_infinite] items-center justify-between whitespace-nowrap rounded-lg px-4 py-2 font-medium text-green-50",
                styles.wave
              )}
            >
            ...
            <div className="flex gap-2 ">
              {orderData.orderStatus.id === 4 && (
                <StatusBadge orderDate={parseJSON(orderData.startDate)} label={orderData.orderStatus.value}>
              ...
              <ContextMenu target=".messages" className="flex cursor-pointer self-center text-transparent group-hover:text-gray-800">
                ...
                <ContextMenu.Content>
                  <a className="block cursor-pointer whitespace-nowrap py-2 pl-4 pr-10 text-sm font-normal text-gray-900 hover:bg-gray-100">
