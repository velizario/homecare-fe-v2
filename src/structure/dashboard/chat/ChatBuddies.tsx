...
export default function ChatBuddies() {
    ...
    return (
        <div className="border-y w-full md:w-auto min-w-0 flex-none">
            {
                <ul role="list" className="relative z-0">
                    {chatBuddies.map((person) => (
                        <li key={person.id} onClick={() => setActive(person.id)} className={classNames(person.id === active ? "bg-indigo-50" : "", "group hover:bg-indigo-50 flex justify-between cursor-pointer border-b border-indigo-100  px-4")}>
