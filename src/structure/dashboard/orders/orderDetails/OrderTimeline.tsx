...
const eventTypes: Record<number, {updateType: string, icon: JSX.Element; style: string }> = {
  ...
  [OrderHistoryLogType.RESERVATION] : {updateType: "Резервация", icon: <PlusIcon className="h-5 w-5 text-white" aria-hidden="true" />, style: "bg-green-400"},
  ...
  [OrderHistoryLogType.ACTIVE] : {updateType: "Променена", icon: <ArrowPathIcon className="h-5 w-5 text-white" aria-hidden="true" />, style: "bg-indigo-400"},
};
...
export default function OrderTimeline({ orderHistory }: OrderTimelineProps) {
  ...
  return (
    <section aria-labelledby="timeline-title" className="xl:col-span-1 xl:col-start-3 ">
      <div className={classNames("bg-white px-4 py-5 shadow sm:rounded-lg sm:px-6")}>
        <h2 id="timeline-title" className="text-base font-medium text-gray-900">
          История
        </h2>
                    ...
                    <div>
                      <span
                        className={classNames(
                          ...
                          "flex h-6 w-6 p-1 items-center justify-center rounded-full ring-8 ring-white"
                        )}
                      >
