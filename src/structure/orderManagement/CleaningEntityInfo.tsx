...
export default function CleaningEntityInfo<K extends FieldValues>({ setValue, setNextStep }: CleaningEntityInfoProps<K>) {
    ...
    <>
      {districtNames.length > 0 && (
        <div className="relative flex w-screen max-w-full flex-col p-8">
          ...
          <p className="mx-auto mt-6 max-w-2xl text-center text-lg leading-8 text-gray-600">Детайли за локация, размер на дома/офиса</p>
          ...
          <div className="mt-10  flex items-end justify-between text-gray-100">
            ...
            <img className="h-20 w-20" src="https://i.pinimg.com/originals/6f/d8/84/6fd884b4cba095b894173481692785cd.gif"></img>
          </div>
          <RangeSlider options={rangeSliderData} activeArea={selectedestateSize} onChange={handleAreaSizeChange} styles="" />
          ...
          <ComboSingleSelectOld selected={selectedDistrict} setSelected={setSelectedDistrict} selections={districtNames} />
          <button
            onClick={handleClick}
            className={classNames(
              selectedestateSize && location ? "bg-indigo-600 font-medium text-white  hover:bg-indigo-500" : "bg-gray-200 font-normal  text-gray-400",
