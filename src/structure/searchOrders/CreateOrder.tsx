import { MinusIcon, PlusCircleIcon, PlusIcon, XMarkIcon } from "@heroicons/react/24/outline";
...
export default function CreateOrder() {
    ...
    return (
        <div className="flex flex-col relative my-10 py-2 px-4 border-t max-w-2xl mx-auto">


            <h2 id="step-1" className="text-xl text-gray-900 mt-10">Каква услуга търсите?</h2>
            ...
            <h2 id="step-2" className="text-xl text-gray-900 mt-10">Ще имате ли нужда от?</h2>
            <Toggle options={additionalServiceChoices[serviceType || "1"]} onClick={handleAdditionalServices} activeId={additionalServices} />
            <h2 id="step-3" className="text-xl text-gray-900 mt-10">Колко често ще са посещенията?</h2>
            <CardChoice options={serviceModeChoices} onClick={handleServiceMode} activeId={serviceMode} styles="grid"></CardChoice>
            <h2 id="step-4" className="text-xl text-gray-900 mt-10">В кои дни искате да са посещенията?</h2>
            ...
            <h2 id="step-5" className="text-xl text-gray-900 mt-10">В кои часове искате да са посещенията?</h2>
            <p className="text-xs text-gray-600">(Изберете повече варианти, ако ви устройват)</p>
            <div className="flex justify-between max-w-md mt-3 px-2">
                <button className="inline-flex items-center rounded bg-white px-2.5 py-1.5 text-xs font-medium text-gray-700 hover:bg-gray-50 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-indigo-500 focus-visible:ring-offset-2" data-id="morning" onClick={handleServiceHoursMultiple}><PlusIcon className="h-3 w-3 -ml-1 mr-0.5 mt-0.5" /> преди обяд</button>
                <button className="inline-flex items-center rounded bg-white px-2.5 py-1.5 text-xs font-medium text-gray-700 hover:bg-gray-50 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-indigo-500 focus-visible:ring-offset-2" data-id="afternoon" onClick={handleServiceHoursMultiple}><PlusIcon className="h-3 w-3 -ml-1 mr-0.5 mt-0.5" />след обяд</button>
                <button className="inline-flex items-center rounded bg-white px-2.5 py-1.5 text-xs font-medium text-gray-700 hover:bg-gray-50 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-indigo-500 focus-visible:ring-offset-2" data-id="clear" onClick={handleServiceHoursMultiple}><XMarkIcon className="h-3 w-3 -ml-1 mr-0.5 mt-0.5" />изчисти</button>
            </div>
            ...
            <h2 id="step-6" className="text-xl text-gray-900 mt-10">Каква площ ще почистваме (кв. м.)?</h2>
            <CardChoice options={areaSizeChoices} onClick={handleAreaSize} activeId={areaSize} styles="grid grid-cols-2"></CardChoice>
            <h2 id="step-7" className="text-xl text-gray-900 mt-10">Къде ще почистваме?</h2>
